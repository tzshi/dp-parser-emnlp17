...
cdef class AHBeamConfiguration:
    ...
    cpdef void initconf(self, int pos, int rootfirst=0):
        cdef int i
        cdef int length = self.length
        self.step[pos] = 0
    ...
    cpdef np.ndarray[ndim=1, dtype=np.npy_intp] goldTransitions(self, int pos, int rootfirst=0):
